...
\cite{chollet2021dlwithpython}, the textbook used in DAT255. The chapters were converted to
...
levels (\texttt{\#}, \texttt{\#\#}, \texttt{\#\#\#}, \texttt{\#\#\#\#}). The second pass
inspects each chunk and, if it exceeds 1\,000 characters, sub-splits it with a 100-character
overlap between consecutive sub-chunks; the overlap preserves context across the boundary
and reduces edge-effect retrieval misses. Each chunk is stored together with breadcrumb
...
model \cite{reimers2020sentence} is supported as a CPU fallback. The embeddings are stored in
...
The retriever uses the BGE tokeniser internally to produce semantic embeddings. The generator
uses a separate tiktoken-based GPT-2 byte-pair encoding tokeniser
(Section~\ref{sec:tokenizer}). The two tokenisers serve different purposes --- the first
exists to compute a similarity vector for search, the second to feed integer IDs into the
generator's embedding layer --- and are deliberately not shared.
