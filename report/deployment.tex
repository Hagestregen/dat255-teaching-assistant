...
only thing that changes between tabs is the task token prepended to the prompt. With
\texttt{--track}, the practice and quiz tabs append per-question outcomes to a local
\texttt{progress.json} so the user can see which topics are still weak.}
\label{fig:app}
\end{figure}

\paragraph{Runtime modes.}
The app supports three CLI configurations. The first (\texttt{--pretrained qwen-3b}) loads
Qwen2.5-3B-Instruct as the generator and runs without progress tracking; this is the
``random study'' mode. The second adds \texttt{--index ../rag/rag\_index} to enable RAG-backed
retrieval. The third additionally passes \texttt{--track} to write a local progress log,
which the flashcard tab uses to surface cards the user has answered incorrectly.
