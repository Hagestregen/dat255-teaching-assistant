...
on the error recorded on the output of `f4`

(the loss of the model).
To adjust `f1`

, you’ll need to percolate error information through `f2`
...
In such cases, use a 1 × 1 `Conv2D`

layer with no activation to linearly project the
...
by the network: even if the data entering a `Dense`
...
Batch normalization does just that. It’s a type of layer (`BatchNormalization`
...
if they include multiple `BatchNormalization`
...
layer can be used after any layer — `Dense`
...
that will make your model smaller (fewer trainable weight parameters),
...
in Keras). This layer performs a spatial convolution on each channel
...
- Your model should be organized into repeated
*blocks*of layers, usually made of multiple convolution layers and a max pooling layer. - The number of filters in your layers should increase as the size of the spatial feature maps decreases.
...
You don’t yet know what Transformers are because we’ll cover them in chapter 15. In short, the Transformer architecture was developed to process text — it’s fundamentally a sequence-processing architecture. And Transformers are very good at it, which has led to the question: could we also use them for images?
