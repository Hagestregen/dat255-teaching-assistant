...
. Given a sequence of all tokens
...
Let’s look at an `(x, y)`

input sample. Our label at each position in the
sequence is the next character in the sequence:

To map this input to a sequence of integers, we can again use the
`TextVectorization`

layer we saw in the last chapter. To learn a character-level
vocabulary instead of a word-level vocabulary, we can change our `split`

argument. Rather than the default `"whitespace"`

splitting, we instead split by
`"character"`

. We will do no standardization here — to keep things simple, we
...
when predicting the label of the current character. We can also use an `Embedding`

, as we saw in the previous chapter, to embed each
...
, which is fast and has a simpler internal state than an `LSTM`
...
vocabulary, and we will `compile()`

it with a crossentropy loss. Note that our
...
cell’s state was handled implicitly when calling the
layer. During generation, we would like to predict a single output token at a
time and explicitly output the state of the `GRU`

’s cell. We need to propagate
...
try replacing the `GRU`

layer with a `Bidirectional(GRU(...))`
...
have seen so far in this book. We cannot simply call `model.predict()`
...
*and*our encoder’s representation of the source sequence.
...
) and uses them to predict the first real token in the sequence. - The predicted sequence so far is fed back into the decoder, in a loop, until
...
Next, let’s prepare two separate `TextVectorization`

layers: one for English and
...
tokens that we’ve inserted. By default, the characters`[`

and`]`

would be stripped, but we want to keep them around so we can distinguish the word`"start"`

from the start token`"[start]"`

. - Punctuation is different from language to language! In the Spanish
`TextVectorization`

layer, if we’re going to strip punctuation characters, we need to also strip the character`¿`
...
Finally, we can turn our data into a `tf.data`

pipeline. We want it to return a
tuple `(inputs, target, sample_weights)`
...
(the tokenized English sentence) and `"spanish"`

(the tokenized
Spanish sentence), and `target`

is the Spanish sentence offset by one step
ahead. `sample_weights`

here will be used to tell Keras which labels to use when
...
nature of RNNs, the model will only look at tokens `0...N`

in the source
sequence to predict token `N`

in the target sequence. Consider translating the
...
sequence. We can achieve this with a `Bidirectional`

layer.

Next, let’s add the decoder — a simple `GRU`

layer that takes as its initial state
the encoded source sentence. On top of it, we add a `Dense`
...
RNN would break training by making the loss function trivially
easy.
...
are already known when predicting token `N + 1`
...
and feed it into the decoder model, together with the
...
each iteration, until we get to `"[end]"`
...
, and so on. Beyond such tweaks, however, the RNN approach to
...
layers in a setup similar to what we just created. However, these
...
paper “Attention Is All You Need.”[1] The
...
sequences through our `GRU`

layers, we will have a vector
...
magnitude. We can achieve this by running the scores through a `softmax`
...
are sequences, the attention scores will be a matrix. Each
...
source and target vectors with `Dense`

layers, the model can find a good
...
`sum(score(target, source) * source)`

. We will write this equivalently with
different input names as `sum(score(query, key) * value)`
...
One way to use the `MultiHeadAttention`

layer would be to add it to our existing
...
dependencies in text that the `GRU`

layer will struggle with. This does, in
...
network — a nonlinear activation function. The `MultiHeadAttention`
...
layers like those we’ve used in image models. That’s
because `BatchNormalization`

doesn’t work well for sequence data. Instead, we’re
using the `LayerNormalization`

layer, which normalizes each sequence
...
collects information from many samples to obtain
...
We also pass a new input to the `MultiHeadAttention`

layer called
`attention_mask`

. This Boolean tensor input will be broadcast to the same shape
as our attention scores `(batch_size, target_length, source_length)`
...
which contain no information. Our encoder layer takes a `source_mask`

input that
will mark all the non-padding tokens in our inputs and upranks it to shape
`(batch_size, 1, source_length)`
...
another `MultiHeadAttention`

layer, which receives both the source and target
...
Our decoder layer takes in both a `target`
...
, we take in a `source_mask`

that marks the location of all
padding in the source input (`True`

for non-padding, `False`

for padding) and use it as an `attention_mask`
...
can be read as a mask for attention for the target token at position `i`
...
lower-triangular mask simply by passing `use_casual_mask`

to the
`MultiHeadAttention`

layer when calling it.
...
model, replacing the `GRU`

layers with our `TransformerEncoder`
...
as the embedding size throughout the model,
except in the feedforward block. In the feedforward
block, we scale up the embedding size to `2048`

before nonlinearity and scale back
...
Our model has almost exactly the same structure as the `GRU`

translation model we
...
“position” axis to the vector and fill it with `0`
...
that varied cyclically depending on the position (they used cosine
...
We would use this `PositionalEmbedding`

layer just like a regular `Embedding`
...
improvement from the `GRU`

model, and that’s all the more impressive when you
...
is a side effect of getting rid of the looped state passing of our `GRU`
...
one go. This makes the `Transformer`

quicker to train on accelerators.

Let’s rerun generation with our newly trained `Transformer`
...
Transformers[2]. The paper and model were released a year
...
replace about 15% of the tokens with a special `[MASK]`

token. The model will
...
predict `p(token|past tokens)`

, the masked language model attempts to predict
...
RoBERTa[3], short for Robustly Optimized BERT. RoBERTa
...
*A matching tokenizer*— Used with the pretrained model itself. Any text must be tokenized in the same way as during pretraining. If the words of our IMDb reviews map to different token indices than they would have during pretraining, we cannot use the learned representations of each token in the model.*A matching model architecture*— To use the pretrained model, we need to recreate the math used internally by the model for pretraining exactly.*The pretrained weights*— These weights were created by training the model for about a day on 1,024 GPUs and billions of input words.

Recreating the tokenizer and architecture code ourselves would not be too hard.
The model internals almost exactly match the `TransformerEncoder`
...
special constructor `from_preset()`

to load a pretrained model’s
...
maps from text to sequences of integers, as we would
expect. Remember the `SubWordTokenizer`

we built in the last chapter? RoBERTa’s
...
we just loaded?
We saw in chapter 8 that a `backbone`

is a term often used in computer vision for a network that maps from
...
sequence and embeds it to an output sequence with shape ```
(batch_size,
sequence_length, 768)
```

, but it’s not set up for a particular loss function. You
...
will download the movie review data to a `train_dir`
...
our translation model. Each sequence would begin with an `<s>`

token, end with an
`</s>`

token, and be followed by any number of `<pad>`
...
provides a layer for this type of token packing called the `StartEndPacker`
...
sequence with shape `(batch_size, sequence_length, 768)`
...
projection with a nonlinearity before generating an output prediction.
...
, a gender vector. This was quite
...
Word2Vec enabled you to do basic things like `plural(cat) → cats`

or
`male_to_female(king) → queen`

. Meanwhile, a large Transformer model can do pure
magic — things like ```
write_this_in_style_of_shakespeare("...your poem...") →
"...new poem..."
```

. And a single model can contain millions of such programs.
...
- A
*language model*is a model that learns a specific probability distribution —`p(token|past tokens)`
...
- A
*masked language model*learns a related probability distribution`p(tokens|surrounding tokens)`

and can be helpful for classifying text and individual tokens. - A
*sequence-to-sequence language model*learns to predict the next token given both past tokens in a target sequence and an entirely separate, fixed source sequence. Sequence-to-sequence models are useful for problems like translation and question answering. - A sequence-to-sequence model usually has two separate components. An
*encoder*computes a representation of the source sequence, and a*decoder*takes this representation as input and predicts the next token in a target sequence based on past tokens.

*Attention*is a mechanism that allows a model to pull information from anywhere in a sequence selectively based on the context of the token currently being processed:- Attention avoids the problems RNNs have with long-range dependencies in text.
...
- The
*Transformer*is a sequence modeling architecture that uses attention as the only mechanism to pass information across a sequence:- The Transformer works by stacking blocks of alternating attention and two-layer feedforward networks.
