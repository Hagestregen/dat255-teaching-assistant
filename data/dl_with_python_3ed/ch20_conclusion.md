...
layers, meant to process vector data (where each sample is a vector
of numerical or categorical attributes).
Such networks assume no specific structure in the input features: they’re called
*densely connected* because the units of a `Dense`

layer are connected to
...
ConvNets covered in chapter 8 typically end with one or two `Dense`
...
layer with a single unit and a `sigmoid`
...
more), end your stack of layers with a `Dense`

layer with a number of units
equal to the number of classes and a `softmax`
...
layer with a number of units equal to the number of
classes and a `sigmoid`
...
end your stack of layers with a `Dense`

layer with a number of units equal to
...
regression — most commonly `mean_squared_error`
...
You can use the `Conv1D`

layer to process
sequences, the `Conv2D`

layer to process images, and the `Conv3D`
...
that turns an input vector sequence into a context-aware, order-aware output vector sequence - A
`TransformerDecoder`

that takes the output of the`TransformerEncoder`

, as well as a target sequence, and predicts what should come next in the target sequence
...
And this is a lone `TransformerEncoder`

for binary classification of integer sequences:
...
. For most practical purposes, you should use either `GRU`
...
is the more powerful of the two but is also more expensive; you
can think of `GRU`

as a simpler, cheaper alternative to it.
