<!-- source: 29_mixture_of_experts_classification.ipynb -->

# Text classification with a Mixture-of-Experts model

This notebook is taken from the [Keras examples](https://keras.io/examples/nlp/text_classification_with_switch_transformer/), and demonstrates a model type that was a head of its time when it was released, but is now very common among state-of-the-art LLMs. The _Switch_ name is gone, but the _Mixture of Experts_ concept is highly relevant.

> This example demonstrates the implementation of the
[Switch Transformer](https://arxiv.org/abs/2101.03961) model for text
classification.
>
> The Switch Transformer replaces the feedforward network (FFN) layer in the standard Transformer with a Mixture of Experts (MoE) routing layer, where each expert operates independently on the tokens in the sequence. This allows increasing the model size without increasing the computation needed to process each example.
>
> Note that, for training the Switch Transformer efficiently, data and model parallelism need to be applied, so that expert modules can run simultaneously, each on its own accelerator. While the implementation described in the paper uses the [TensorFlow Mesh](https://github.com/tensorflow/mesh) framework for distributed training, this example presents a simple, non-distributed implementation of the Switch Transformer
...
We can download it directly from `keras.datasets` this time.
...
To be able to decode the data later, we also need to get the dictionary from word index back to the actual word.
...
```python
preds = classifier.predict(x_val, batch_size=batch_size)

for x, y, pred in zip(x_val[:10], y_val[:10], preds):
    text = " ".join(inverted_word_index[i] for i in x)
    text = text.strip()
    print(text)
    print(f"Predicted: {pred.argmax()}, Actual: {y}")
    print()
```
